...
function App() {
  ...
  const pdf = async () => {
    ...
    // Важно: НЕ меняем общую раскладку страницы на время снимка.
    // Раньше класс pdf-exporting скрывал toolbar, из-за этого в Chrome менялась
    // высота рабочей области, фон фасада пересчитывался через object-fit,
    // а вывеска оставалась в старых координатах и в PDF визуально съезжала.
    // Теперь фиксируем текущий размер stage и скрываем только служебные элементы внутри него.
    const stageEl = stageRef.current;
    const stageRectBeforeExport = stageEl.getBoundingClientRect();
    const prevStageWidth = stageEl.style.width;
    const prevStageHeight = stageEl.style.height;
    const prevStageMinHeight = stageEl.style.minHeight;
    const prevStageMaxHeight = stageEl.style.maxHeight;
    const prevStageFlex = stageEl.style.flex;
    stageEl.style.width = `${stageRectBeforeExport.width}px`;
    stageEl.style.height = `${stageRectBeforeExport.height}px`;
    stageEl.style.minHeight = `${stageRectBeforeExport.height}px`;
    stageEl.style.maxHeight = `${stageRectBeforeExport.height}px`;
    stageEl.style.flex = `0 0 ${stageRectBeforeExport.height}px`;
    ...
    try {
      ...
      stageEl.style.width = prevStageWidth;
      stageEl.style.height = prevStageHeight;
      stageEl.style.minHeight = prevStageMinHeight;
      stageEl.style.maxHeight = prevStageMaxHeight;
      stageEl.style.flex = prevStageFlex;
    }
