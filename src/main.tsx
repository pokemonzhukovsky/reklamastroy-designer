...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}mm" height="${h}mm" viewBox="0 0 ${w} ${h}" version="1.1">
  ...
  const pdf = async () => {
    ...
    // v3.70 — КРИТИЧНО: первую страницу PDF снимаем ИЗ ТЕКУЩЕЙ живой геометрии
    // до любых принудительных загрузок шрифтов и пересчётов метрик. Раньше экспорт сам
    // вызывал setFontMeasureTick(), из-за чего ширина/высота текста могла поменяться прямо
    // перед html2canvas. При этом signPos оставался прежним, и вывеска на фасаде в PDF
    // оказывалась немного сдвинутой относительно того места, которое видел пользователь.
    // Теперь визуализация фасада фиксируется первой и больше не меняется во время экспорта.
