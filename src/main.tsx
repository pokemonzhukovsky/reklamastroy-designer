  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    >
      ...
      <svg className="signDims" width={widthPx + 112} height={heightPx + 88} viewBox={`0 0 ${widthPx + 112} ${heightPx + 88}`}>
        ...
        <text x="25" y={heightPx / 2 + 54} textAnchor="middle" transform={`rotate(-90 25 ${heightPx / 2 + 54})`}>{mm(signHeightCm)}</text>
      </svg>
  ...
  return <>
    ...
    <main className="app">
      <aside className="panel left">
        ...
        <div className="summaryCard leftSummary"><b>Габариты</b><span>Длина: {mm(signWidthCm)}</span><span>Высота: {mm(signHeightCm)}</span><span>1 строка: {mm(line1HeightCm)}</span>{twoLines && <span>2 строка: {mm(line2HeightCm)}</span>}{logo && <span>Высота логотипа: {mm(logoHeightCm)}</span>}<span>Подсветка: {lightingLabel(lighting)}</span></div>
