...
function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedLine1Height = st.line1HeightMm ?? st.line1HeightCm ?? defaultState.line1HeightCm;
      const savedLine2Height = st.line2HeightMm ?? st.line2HeightCm ?? Math.round(savedLine1Height * 0.8);
      const savedCalibration = st.calibrationMm ?? st.calibrationCm ?? defaultState.calibrationCm;
      setLine1HeightCm(savedLine1Height);
      setLine1HeightInput(String(savedLine1Height));
      setLine2HeightCm(savedLine2Height);
      setLine2HeightInput(String(savedLine2Height));
      setCalibrationCm(savedCalibration);
      setCalibrationInput(String(savedCalibration));
      ...
      const savedLogoHeight = st.logoHeightMm ?? (st.logoHeightCm ? st.logoHeightCm * 10 : defaultState.logoHeightCm);
      setLogoHeightCm(savedLogoHeight);
      setLogoHeightInput(String(savedLogoHeight));
      setLogoPosition(['left','right','top','bottom'].includes(st.logoPosition) ? st.logoPosition : defaultState.logoPosition);
      setPoints(st.points ?? []);
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    const dimSvgWidth = widthPx + (corelDims ? 360 : 140);
    ...
    const lineAbs = lines.map((line, i) => {
      ...
      return {
        left: dimSignX + textStartX,
        right: dimSignX + textStartX + (lineWidthsScaled[i] || 0),
        top: dimTopY + top,
    ...
    const drawHDim = (key: string, x1: number, x2: number, y: number, objectY: number, label: string, labelOffset = -9) => <React.Fragment key={key}>
    ...
    const drawVDim = (key: string, x: number, y1: number, y2: number, objectX: number, label: string, labelSide: 'left' | 'right' = 'left') => {
      const textX = labelSide === 'left' ? x - 18 : x + 18;
    ...
    >
      ...
      >
        ...
        {drawingMode && pdfMode ? <>
          {/* Общая длина вывески */}
          {drawHDim('sign-width', signAbsLeft, signAbsRight, dimHorizontalY, signAbsTop, mm(signWidthCm), -9)}

          {/* Общая высота вывески */}
          {drawVDim('sign-height', signAbsLeft - 92, signAbsTop, signAbsBottom, signAbsLeft, mm(displayHeightMm), 'left')}

          {/* Габариты логотипа */}
          {logo && <>
            {drawHDim('logo-width', logoAbsLeft, logoAbsRight, logoAbsTop - 22, logoAbsTop, mm(logoWidthMm), -8)}
            {drawVDim(
              'logo-height',
              logoPosition === 'left' ? logoAbsLeft - 24 : logoAbsRight + 24,
              logoAbsTop,
              logoAbsBottom,
          ...
          {/* Высота заглавных букв первой строки */}
          {lineAbs[0] && drawVDim('line1-cap-height', lineAbs[0].right + 24, lineAbs[0].top, lineAbs[0].bottom, lineAbs[0].right, mm(line1HeightCm), 'right')}

          {/* Высота строчных букв первой строки */}
          {lineAbs[0]?.hasLower && drawVDim('line1-lower-height', lineAbs[0].right + 50, lineAbs[0].bottom - lineAbs[0].lowerHeight, lineAbs[0].bottom, lineAbs[0].right, mm(lineAbs[0].lowerMm), 'right')}

          {/* Габариты второй строки */}
          {twoLines && lines[1]?.trim() && lineAbs[1] && <>
            {drawHDim('line2-width', lineAbs[1].left, lineAbs[1].right, lineAbs[1].bottom + 22, lineAbs[1].bottom, mm(line2WidthMm), 26)}
            {drawVDim('line2-cap-height', lineAbs[1].right + 24, lineAbs[1].top, lineAbs[1].bottom, lineAbs[1].right, mm(line2HeightMm), 'right')}
            {lineAbs[1].hasLower && drawVDim('line2-lower-height', lineAbs[1].right + 50, lineAbs[1].bottom - lineAbs[1].lowerHeight, lineAbs[1].bottom, lineAbs[1].right, mm(lineAbs[1].lowerMm), 'right')}
          </>}
        </> : <>
      ...
      <div className="letters" style={lettersStyle} data-lighting={lighting}>
        ...
        <div className="textBlock" style={{ width: textWidthPxScaled }}>{lines.map((line, i) => <span key={i} style={{ fontSize: fontSizes[i], lineHeight: `${lineTargetHeightsPx[i] * scale}px`, minHeight: `${lineTargetHeightsPx[i] * scale}px`, ...lightingTextStyle(fontSizes[i]) }}>{line}</span>)}</div>
  ...
  return <>
    ...
    <main className="app">
      <aside className="panel left panelPremium">
        <div className="sectionBadge">Укажите реальный размер на фото</div>
        ...
        <label>Фактический размер между точками, мм<input type="number" value={calibrationInput} onChange={e => handleSizeInput(e.target.value, setCalibrationInput, setCalibrationCm)} /></label>
