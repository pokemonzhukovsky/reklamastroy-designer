    ...
    {downloadGateOpen && createPortal(
      <div className="leadGateOverlay" role="dialog" aria-modal="true" aria-labelledby="lead-gate-title" onMouseDown={(e) => {
        ...
        <div className="leadGateModal">
          ...
          <p>Введите имя и телефон. После успешной отправки заявки файл {pendingDownload === 'svg' ? 'SVG' : 'PDF'} начнёт скачиваться автоматически.</p>
          <form onSubmit={submitLeadForDownload}>
            ...
            <button type="submit" className="leadGateSubmit" disabled={leadSending}>
              {leadSending ? 'Отправляем заявку…' : `Отправить и скачать ${pendingDownload === 'svg' ? 'SVG' : 'PDF'}`}
            </button>
          </form>
          <small>Заявка будет передана в РекламаСтрой для связи по вашему проекту.</small>
        </div>
      </div>,
      document.body
    )}
