...
function ColorDropdown({ label, palette, list, value, onChange, dropUp = false }: {
  label: string;
  palette: string;
  list: ColorItem[];
  value: string;
  ...
  return <div className={`colorDropdown ${palette === 'ORACAL 8500' ? 'colorDropdownFace' : 'colorDropdownSide'} ${dropUp ? 'colorDropdownDropUp' : ''} ${open ? 'isOpen' : ''}`} ref={wrapRef}>
...
function FontPicker({ label, value, onChange, open, setOpen, preview }: {
  ...
  return <>
    ...
    <div className="fontDropdown">
      <button className="fontCurrent" type="button" onClick={() => setOpen(v => !v)}>
  ...
  return <>
    <main className="app constructorFrame">
      <aside className="panel left panelPremium">
        ...
        {twoLines && <ColorDropdown label="Лицевая часть второй строки" palette="ORACAL 8500" list={ORACAL_8500} value={line2Face} onChange={setLine2Face} dropUp />}
        <ColorDropdown label={twoLines ? 'Борт первой строки' : 'Борта'} palette="ORACAL 641" list={ORACAL_641} value={side} onChange={setSide} />
        {twoLines && <ColorDropdown label="Борт второй строки" palette="ORACAL 641" list={ORACAL_641} value={line2Side} onChange={setLine2Side} dropUp />}
