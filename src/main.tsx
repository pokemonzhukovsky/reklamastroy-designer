...
function ColorDropdown({ label, palette, list, value, onChange, showCodes = true }: {
  label: string;
  palette: string;
  list: ColorItem[];
  value: string;
...
function App() {
  ...
  const getGlyphMetrics = (text: string, fontSizePx: number, fontFamily = font) => {
    ...
    if (typeof document === 'undefined') {
      ...
      return { width: fallbackWidth, height: fontSizePx * 0.72, left: 0, right: fallbackWidth, visualWidth: fallbackWidth };
    }
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    if (!ctx) {
    ...
    const visualWidth = Math.max(1, left + right);
    return {
  ...
  const textLineOffsetsPx = lines.map((_, i) => {
    // Canvas actualBoundingBoxLeft показывает, насколько глиф выходит влево
    // относительно точки начала текста. Чтобы визуальное начало 1-й и 2-й строки
    // совпадало, точку начала каждой строки сдвигаем вправо ровно на её left-bearing.
    const currentWidth = textLineWidthsPx[i] || 0;
    const leftBearing = Math.max(0, lineTextMetrics[i]?.left || 0);
    if (!twoLines || textLineWidthsPx.length < 2) return leftBearing;
    if (line2Align === 'center') return leftBearing + Math.max(0, (textWidthPx - currentWidth) / 2);
    if (line2Align === 'right') return leftBearing + Math.max(0, textWidthPx - currentWidth);
    return leftBearing;
  });
  ...
  const contentHeightMm = Math.max(1, Math.round(signHeightPx / workingPxPerCm));
  ...
  const drawingScale = Math.min(1.16, 780 / Math.max(1, visualSignWidthPx + 520), 390 / Math.max(1, visualSignHeightPx + 260));
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    // Борт подложки соответствует выбранному цвету. В режиме «ночь» подложка
    // не светится и не получает белые/цветные ореолы — она только затемняется.
    const substrateNightColor = shadeHex(substrateColor, -130);
    const substrateVisibleColor = night ? substrateNightColor : substrateColor;
    const substrateEdgeColor = night ? shadeHex(substrateColor, -150) : substrateColor;
    const substrateDepth = Math.max(8, Math.min(36, Math.min(widthPx, heightPx) * 0.09));
    const hardShadow = night ? 'rgba(0,0,0,.55)' : 'rgba(0,0,0,.28)';
    const midShadow = night ? 'rgba(0,0,0,.62)' : 'rgba(0,0,0,.34)';
    const softShadow = night ? 'rgba(0,0,0,.72)' : 'rgba(0,0,0,.30)';
    const substrateInset = night
      ? `inset -${Math.max(1, substrateDepth * 0.16)}px -${Math.max(1, substrateDepth * 0.16)}px 0 rgba(0,0,0,.24)`
      : `inset ${Math.max(1, substrateDepth * 0.12)}px ${Math.max(1, substrateDepth * 0.12)}px 0 rgba(255,255,255,.22), inset -${Math.max(1, substrateDepth * 0.18)}px -${Math.max(1, substrateDepth * 0.18)}px 0 rgba(0,0,0,.12)`;
    const substrateShadow = `${substrateDepth * 0.55}px ${substrateDepth * 0.55}px 0 ${hardShadow}, ${substrateDepth * 0.95}px ${substrateDepth * 1.1}px ${substrateDepth * 1.25}px ${midShadow}, ${substrateDepth * 1.45}px ${substrateDepth * 1.75}px ${substrateDepth * 2.25}px ${softShadow}, ${substrateInset}`;
    ...
    const dimSvgWidth = widthPx + (corelDims ? 860 : 170);
    const dimSvgHeight = heightPx + (corelDims ? 430 : 120);
    ...
    const lineAbs = lines.map((line, i) => {
      ...
      const leftBearing = Math.max(0, (lineTextMetrics[i]?.left || 0) * scale);
      const visualLeft = dimSignX + textStartX + (lineOffsetsScaled[i] || 0) - leftBearing;
      const visualWidth = lineWidthsScaled[i] || 0;
      return {
        left: visualLeft,
    ...
    const rightDimBase = Math.max(substrateAbsRight, signAbsRight) + (logo && logoPosition !== 'left' ? 190 : 120);
    const rightDimGap = corelDims ? 118 : 48;
    ...
    const line2WidthDimY = twoLines && lineAbs[1] ? lineAbs[1].bottom + 52 : signAbsBottom + 46;
    const signWidthDimY = substrateEnabled
      ? (twoLines && lineAbs[1] ? Math.max(signAbsBottom + 82, line2WidthDimY + 70) : signAbsBottom + 68)
      : dimHorizontalY;
    return <div
    ...
    >
      ...
      >
        ...
        {drawingMode && pdfMode ? <>
          {substrateEnabled && drawHDim('substrate-width', substrateAbsLeft, substrateAbsRight, dimHorizontalY, substrateAbsTop, mm(displayWidthMm), -10)}
          {substrateEnabled && drawVDim('substrate-height', substrateHeightDimX, substrateAbsTop, substrateAbsBottom, substrateAbsLeft, mm(displayHeightMm), 'left', { textOffset: 22 })}
          {drawHDim('sign-width', signAbsLeft, signAbsRight, signWidthDimY, signAbsTop, mm(contentWidthMm), substrateEnabled ? 31 : -10)}
          {drawVDim('sign-height', signHeightDimX, signAbsTop, signAbsBottom, signAbsRight, mm(contentHeightMm), substrateEnabled ? 'right' : 'left', { textOffset: 22 })}
          {logo && <>
          ...
          {lineAbs[0] && drawVDim('line1-cap-height', line1CapX, lineAbs[0].top, lineAbs[0].bottom, lineAbs[0].right, mm(line1HeightCm), 'right', { horizontalLabel: true, labelY: lineAbs[0].top + 18, textOffset: 14 })}
          {lineAbs[0]?.hasLower && drawVDim('line1-lower-height', line1LowerX, lineAbs[0].bottom - lineAbs[0].lowerHeight, lineAbs[0].bottom, lineAbs[0].right, mm(lineAbs[0].lowerMm), 'right', { horizontalLabel: true, labelY: lowerLabelY(lineAbs[0]), textOffset: 14 })}
          {twoLines && lines[1]?.trim() && lineAbs[1] && <>
            {drawHDim('line2-width', lineAbs[1].left, lineAbs[1].right, line2WidthDimY, lineAbs[1].bottom, mm(line2WidthMm), 31)}
            {drawVDim('line2-cap-height', line2CapX, lineAbs[1].top, lineAbs[1].bottom, lineAbs[1].right, mm(line2HeightMm), 'right', { horizontalLabel: true, labelY: lineAbs[1].top + 18, textOffset: 14 })}
            {lineAbs[1].hasLower && drawVDim('line2-lower-height', line2LowerX, lineAbs[1].bottom - lineAbs[1].lowerHeight, lineAbs[1].bottom, lineAbs[1].right, mm(lineAbs[1].lowerMm), 'right', { horizontalLabel: true, labelY: lowerLabelY(lineAbs[1]), textOffset: 14 })}
          </>}
        </> : <>
          ...
          {drawHDim('live-sign-width', signAbsLeft, signAbsRight, substrateEnabled ? signAbsBottom + 30 : dimHorizontalY, signAbsTop, mm(contentWidthMm), substrateEnabled ? 25 : -8)}
          {drawVDim('live-sign-height', substrateEnabled ? substrateAbsRight + 22 : signAbsLeft - 22, signAbsTop, signAbsBottom, signAbsRight, mm(contentHeightMm), substrateEnabled ? 'right' : 'left')}
        </>}
      </svg>
      <div className="letters" style={lettersStyle} data-lighting={lighting}>
        {substrateEnabled && <div className="signSubstrate" style={{
          width: widthPx,
          height: heightPx,
    ...
    <main className="app constructorFrame">
      <aside className="panel left panelPremium">
        ...
        {substrateEnabled && <>
          ...
          <ColorDropdown label="Цвет подложки" palette="ORACAL 641" list={ORACAL_641} value={substrateColor} onChange={setSubstrateColor} showCodes={false} />
        ...
        <div className="pdfDrawingGrid centeredLayout">
          ...
          <aside className="pdfSpec pdfSpecHorizontal">
            <div className="pdfSpecItem">
              ...
              <span>{mm(contentWidthMm)} × {mm(contentHeightMm)}</span>
            </div>
            <div className="pdfSpecItem">
            ...
            {substrateEnabled && <div className="pdfSpecItem">
              <b>РАЗМЕР ПОДЛОЖКИ</b>
              <span>{mm(displayWidthMm)} × {mm(displayHeightMm)}</span>
