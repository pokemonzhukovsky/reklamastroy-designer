...
function App() {
  ...
  const pdf = async () => {
    ...
    // ВАЖНО: не трогаем layout страницы и не скрываем верхний toolbar/body-классом.
    // Если при экспорте меняется высота или ширина рабочей области, Chrome заново
    // пересчитывает object-fit у фотографии фасада, а вывеска остаётся в старых
    // координатах. Из-за этого в PDF она визуально съезжает относительно фасада.
    // Поэтому снимаем именно тот stage, который видит пользователь, и скрываем
    // только служебные элементы внутри stage через visibility, без display:none.
    const stageEl = stageRef.current;
    if ((document as any).fonts?.ready) {
      await (document as any).fonts.ready.catch(() => undefined);
    }

    // Запоминаем реальное положение ФАКТИЧЕСКОЙ фотографии фасада внутри рабочей области ДО экспорта.
    // Важно: getBoundingClientRect() у <img> возвращает весь DOM-блок, а не реальную область картинки
    // при object-fit: contain. Поэтому отдельно считаем прямоугольник видимого изображения по naturalWidth/naturalHeight.
    // Дальше в PDF вставляется уже готовый растровый снимок этой области — фасад не растягивается повторно,
    // а вывеска остаётся ровно на тех же координатах, что и на экране.
    ...
    try {
      ...
      stageCanvas = await html2canvas(stageEl, {
        scale: 2.5,
        useCORS: true,
    ...
    // Вырезаем из снимка stage только фактическую видимую фотографию фасада вместе с вывеской.
    // Это убирает серые поля рабочей области и не даёт PDF-шаблону заново растягивать исходное фото.
    let exportCanvas = stageCanvas;
    if (liveFacadeRect && liveStageRect.width > 0 && liveStageRect.height > 0 && liveFacadeRect.width > 10 && liveFacadeRect.height > 10) {
      const scaleX = stageCanvas.width / liveStageRect.width;
      const scaleY = stageCanvas.height / liveStageRect.height;
      const sx = clamp(Math.round((liveFacadeRect.left - liveStageRect.left) * scaleX), 0, stageCanvas.width - 1);
      const sy = clamp(Math.round((liveFacadeRect.top - liveStageRect.top) * scaleY), 0, stageCanvas.height - 1);
      const sw = clamp(Math.round(liveFacadeRect.width * scaleX), 1, stageCanvas.width - sx);
      const sh = clamp(Math.round(liveFacadeRect.height * scaleY), 1, stageCanvas.height - sy);
      if (sw > 20 && sh > 20) {
        ...
        if (croppedCtx) {
          ...
          croppedCtx.fillRect(0, 0, sw, sh);
          croppedCtx.drawImage(stageCanvas, sx, sy, sw, sh, 0, 0, sw, sh);
