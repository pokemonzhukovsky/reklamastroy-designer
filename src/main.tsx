...
function App() {
  ...
  const maxLineLeftBearingPx = Math.max(0, ...lineTextMetrics.map(metric => metric.left || 0));
  const textLineBaseOffsetsPx = lineTextMetrics.map(metric => Math.max(0, maxLineLeftBearingPx - (metric.left || 0)));
  const textWidthPx = Math.max(1, ...textLineWidthsPx.map((width, i) => width + (textLineBaseOffsetsPx[i] || 0)));
  const textLineOffsetsPx = lines.map((_, i) => {
    const currentWidth = textLineWidthsPx[i] || 0;
    const baseOffset = textLineBaseOffsetsPx[i] || 0;
  ...
  const pdf = async () => {
    ...
    if (liveFacadeImg && liveFacadeRect && liveStageRect.width > 0 && liveStageRect.height > 0 && liveFacadeRect.width > 10 && liveFacadeRect.height > 10) {
      ...
      const isNightMode = stageEl.classList.contains('night');
      exportCtx.save();
      if (isNightMode) {
