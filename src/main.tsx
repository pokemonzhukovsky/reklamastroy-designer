...
function App() {
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    const hardShadow = substrateNightOnly ? 'rgba(0,0,0,.62)' : 'rgba(0,0,0,.28)';
    const midShadow = substrateNightOnly ? 'rgba(0,0,0,.72)' : 'rgba(0,0,0,.34)';
    const softShadow = substrateNightOnly ? 'rgba(0,0,0,.82)' : 'rgba(0,0,0,.30)';
    const substrateInset = substrateNightOnly
