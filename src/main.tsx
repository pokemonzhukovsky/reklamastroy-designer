...
function App() {
  ...
  const drawingScale = Math.min(1.0, 650 / Math.max(1, signWidthPx + 130), 215 / Math.max(1, signHeightPx + 90));
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    >
      ...
      <svg className="signDims" width={widthPx + 140} height={heightPx + 88} viewBox={`0 0 ${widthPx + 140} ${heightPx + 88}`}>
        <defs>
        ...
        {drawingMode && pdfMode ? <>
          ...
          <line className="dimExt" x1="48" y1={48 + uppercaseHeightPx * scale} x2="70" y2={48 + uppercaseHeightPx * scale} />
          <line x1="48" y1="48" x2="48" y2={48 + uppercaseHeightPx * scale} markerStart={`url(#arrow-start-${pdfMode ? 'pdf' : 'live'})`} markerEnd={`url(#arrow-end-${pdfMode ? 'pdf' : 'live'})`} />
          <text x="25" y={(48 + (uppercaseHeightPx * scale) / 2) + 4} textAnchor="middle" transform={`rotate(-90 25 ${(48 + (uppercaseHeightPx * scale) / 2) + 4})`}>{mm(uppercaseHeightMm)}</text>

          {hasLowercaseLetters && <>
            <line className="dimExt" x1="22" y1={48 + (uppercaseHeightPx - lowercaseHeightPx) * scale} x2="70" y2={48 + (uppercaseHeightPx - lowercaseHeightPx) * scale} />
            <line className="dimExt" x1="22" y1={48 + uppercaseHeightPx * scale} x2="70" y2={48 + uppercaseHeightPx * scale} />
            <line x1="22" y1={48 + (uppercaseHeightPx - lowercaseHeightPx) * scale} x2="22" y2={48 + uppercaseHeightPx * scale} markerStart={`url(#arrow-start-${pdfMode ? 'pdf' : 'live'})`} markerEnd={`url(#arrow-end-${pdfMode ? 'pdf' : 'live'})`} />
            <text x="-1" y={(48 + uppercaseHeightPx * scale - (lowercaseHeightPx * scale) / 2) + 4} textAnchor="middle" transform={`rotate(-90 -1 ${(48 + uppercaseHeightPx * scale - (lowercaseHeightPx * scale) / 2) + 4})`}>{mm(lowercaseHeightMm)}</text>
          </>}
        </> : <>
          {drawingMode && <>
            <line className="dimExt" x1="70" y1="26" x2="70" y2="48" />
  ...
  return <>
    ...
    <main className="app">
      <aside className="panel left">
        ...
        <div className="summaryCard leftSummary"><b>Габариты</b><span>Длина: {mm(signWidthCm)}</span><span>Высота: {mm(displayHeightMm)}</span><span>1 строка: {mm(line1HeightCm)}</span>{twoLines && <span>2 строка: {mm(line2HeightCm)}</span>}{logo && <span>Высота логотипа: {mm(logoHeightCm)}</span>}<span>Подсветка: {lightingLabel(lighting)}</span></div>
    ...
    <div className="pdfPages" ref={pdfRef}>
      <section className="pdfSheet pdfSheetSingle">
        <header className="pdfTopHeader">
          ...
          <div className="pdfTrust"><i>▣</i><span><b>12 ЛЕТ</b><br/>ОПЫТА</span></div>
          <div className="pdfTrust"><i>☆</i><span><b>1300+</b><br/>РЕАЛИЗОВАННЫХ ПРОЕКТОВ</span></div>
          <div className="pdfContact"><span>🌐 reklamastroy.ru</span><span>✉ sales@reklamastroy.ru</span></div>
          <div className="pdfContact phones"><span>☎ +7 495 008 37 95</span><span>📱 +7 925 888 37 95</span></div>
        </header>
        <h2 className="pdfSheetTitle">Визуализация вывески</h2>
        <div className="pdfFacadeCenter pdfFacadeCenterCompact">
          <div className="pdfFacadeStage pdfFacadeStageCompact snapshotStage">
            {stageShot
        ...
        <h2 className="pdfSheetTitle pdfSheetTitleLower">Макет вывески</h2>
        <div className="pdfDrawingGrid pdfDrawingGridCompact">
          ...
          <aside className="pdfSpec pdfSpecCompact">
