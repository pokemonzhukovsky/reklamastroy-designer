...
function App() {
  ...
  const [centerLine2, setCenterLine2] = useState(defaultState.centerLine2);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setCenterLine2(Boolean(st.centerLine2));
      setFacade(st.facade ?? '');
      setLogo(st.logo ?? '');
  ...
  useEffect(() => {
    const payload = { unit: 'mm', line1, line2, font, face, side, line1HeightMm: line1HeightCm, line2HeightMm: line2HeightCm, calibrationMm: calibrationCm, lighting, night, twoLines, centerLine2, facade, logo, logoAspect, logoHeightMm: logoHeightCm, logoPosition, points, signPos, rotation };
    const t = setTimeout(() => {
      localStorage.setItem('rsDesignerProjectV318MM', JSON.stringify(payload));
    }, 700);
    return () => clearTimeout(t);
  }, [line1, line2, font, face, side, line1HeightCm, line2HeightCm, calibrationCm, lighting, night, twoLines, centerLine2, facade, logo, logoAspect, logoHeightCm, logoPosition, points, signPos, rotation]);
  ...
  const centerSecondLine = twoLines && centerLine2 && textLineWidthsPx.length > 1;
  const textLineOffsetsPx = lines.map((_, i) => {
    if (!centerSecondLine) return 0;
    const firstWidth = textLineWidthsPx[0] || 0;
    const secondWidth = textLineWidthsPx[1] || 0;
    if (i === 0) return Math.max(0, (secondWidth - firstWidth) / 2);
    if (i === 1) return Math.max(0, (firstWidth - secondWidth) / 2);
    return 0;
  });
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    >
      ...
      <div className="letters" style={lettersStyle} data-lighting={lighting}>
        ...
        <div className="textBlock" style={{ width: textWidthPxScaled }}>{lines.map((line, i) => <span key={i} style={{ marginLeft: `${lineOffsetsScaled[i] || 0}px`, fontSize: fontSizes[i], lineHeight: `${lineTargetHeightsPx[i] * scale}px`, minHeight: `${lineTargetHeightsPx[i] * scale}px`, ...lightingTextStyle(fontSizes[i]) }}>{line}</span>)}</div>
  ...
  return <>
    <main className="app constructorFrame">
      <aside className="panel left panelPremium">
        ...
        {twoLines && <label className="check"><input type="checkbox" checked={centerLine2} onChange={e => setCenterLine2(e.target.checked)} /> Центрировать вторую строку</label>}
      ...
      <section className="pdfSheet pdfSheetFacade">
        ...
        <div className="pdfFacadeCenter">
          <div className="pdfFacadeStage snapshotStage">
            {stageShot
              ? <div className="stageSnapshotBox" style={stageShotRatio >= 845 / 500 ? { aspectRatio: String(stageShotRatio), width: '100%' } : { aspectRatio: String(stageShotRatio), height: '100%' }}>
                  <img src={stageShot} className="stageSnapshot"/>
                  <div className="pdfWatermark">REKLAMASTROY.RU</div>
                </div>
              : (facade ? <div className="stageSnapshotBox"><img src={facade} className="facade"/><div className="pdfWatermark">REKLAMASTROY.RU</div></div> : <div className="placeholder">Фото фасада не загружено</div>)}
          </div>
        </div>
