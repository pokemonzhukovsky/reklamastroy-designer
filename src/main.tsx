...
function primaryFontName(family: string) {
...
function recolorTracedSvg(svg: string, fill: string) {
  return svg
...
function App() {
  ...
  const downloadVectorSvg = async () => {
    ...
    const textLineWidthsMm = textLineWidthsPx.map(toMm);
    ...
    const traceTextLayer = (text: string, family: string, fontMm: number, xMm: number, yMm: number, color: string, extraOffsetMm = 0) => {
      const line = text && text.trim() ? text : ' ';
      const widthMm = Math.max(2, (toMm(getGlyphMetrics(line, fontMm * workingPxPerCm, family).visualWidth || getGlyphMetrics(line, fontMm * workingPxPerCm, family).width)));
      const padMm = Math.max(3, fontMm * 0.18);
      const widthPx = Math.max(32, Math.ceil((widthMm + padMm * 2 + extraOffsetMm) * pixelsPerMm));
      const heightPx = Math.max(32, Math.ceil((fontMm * 1.4 + padMm * 2 + extraOffsetMm) * pixelsPerMm));
      const canvas = document.createElement('canvas');
      ...
      ctx.textAlign = 'left';
      ctx.font = `900 ${fontMm * pixelsPerMm}px ${family}`;
      ctx.fillText(line, (padMm + extraOffsetMm) * pixelsPerMm, (padMm + extraOffsetMm) * pixelsPerMm);
      ...
      const inner = recolorTracedSvg(extractSvgInner(tracedSvg), color);
      return `<g transform="translate(${fmt(xMm - padMm)} ${fmt(yMm - padMm)}) scale(${fmt(1 / pixelsPerMm)})">${inner}</g>`;
    };
    ...
    const textNodes = lines.map((line, i) => {
      ...
      const family = primaryFontName(lineFonts[i] || font);
      const fill = lineFaces[i] || face;
      const stroke = lineSides[i] || side;
      const back = traceTextLayer(line, family, fontMm, x + sideDepthMm, y + sideDepthMm, stroke, sideDepthMm);
      const front = traceTextLayer(line, family, fontMm, x, y, fill, 0);
      return `
    <g id="text-line-${i + 1}">${back}${front}</g>`;
    }).join('');
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}mm" height="${h}mm" viewBox="0 0 ${w} ${h}" version="1.1">
  ...
  <desc>SVG с кривыми. Размеры заданы в миллиметрах.</desc>
  <g id="sign-layout">
    ${substrateSvg}
    ${logoSvg}
    ${textNodes}
    ...
    downloadTextFile('reklamastroy-maket-curves.svg', svg);
  };
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false, snapshotSrc = '' }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean; snapshotSrc?: string }) => {
    ...
    const shouldUseSnapshot = !!snapshotSrc && pdfMode && drawingMode;
    return <div
    ...
    >
      ...
      <div className="letters" style={lettersStyle} data-lighting={lighting} ref={!pdfMode && !drawingMode ? liveLettersRef : undefined}>
        {shouldUseSnapshot
          ? <img className="signSnapshotGraphic" src={snapshotSrc} alt="" draggable={false} style={{ width: widthPx, height: heightPx }} />
          : <>
  ...
  return <>
    <main className="app constructorFrame">
      <aside className="panel left panelPremium">
        ...
        <button className="panelPdfButton panelSvgButton" onClick={downloadVectorSvg}>Скачать SVG с кривыми</button>
      </aside>
      ...
      <section className="pdfSheet pdfSheetDrawing">
        ...
        <div className="pdfDrawingGrid centeredLayout">
          <div className="pdfDrawingStage">
            <SignGraphic pdfMode drawingMode scale={drawingScale} snapshotSrc={pdfSignShot}/>
