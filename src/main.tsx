...
type DragMode = null | { type: 'sign'; dx: number; dy: number } | { type: 'point'; index: number } | { type: 'rotate' };
...
function App() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem('rsDesignerProjectV32');
    if (!raw) return;
    try {
  ...
  useEffect(() => {
    const payload = { text, font, face, side, letterHeightCm, calibrationCm, lighting, night, twoLines, facade, logo, points, signPos, rotation };
    const t = setTimeout(() => {
      localStorage.setItem('rsDesignerProjectV32', JSON.stringify(payload));
      setSavedAt(new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }));
    }, 500);
    return () => clearTimeout(t);
  }, [text, font, face, side, letterHeightCm, calibrationCm, lighting, night, twoLines, facade, logo, points, signPos, rotation]);
  ...
  useEffect(() => {
    const move = (e: MouseEvent) => {
      ...
      if (drag.type === 'rotate') {
        ...
        setRotation(Math.round(Math.atan2(p.y - cy, p.x - cx) * 180 / Math.PI));
      }
    };
  ...
  const logoWidthCm = logo ? letterHeightCm * 1.12 : 0;
  const textWidthCm = maxLineLength(lines) * letterHeightCm * 0.62;
  const gapCm = logo ? letterHeightCm * 0.18 : 0;
  ...
  const signHeightCm = twoLines ? Math.round(letterHeightCm * 1.92) : letterHeightCm;
  const signHeightPx = exact ? signHeightCm * pxPerCm : (twoLines ? 124 : 64);
  ...
  const stageScaleForPdf = 845 / Math.max(1, stageSize.width);
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1 }: { pdfMode?: boolean; scale?: number }) => {
    ...
    return <div className={pdfMode ? 'sign signPdf' : 'sign'} style={{ left: pdfMode ? signPos.x * scale : signPos.x, top: pdfMode ? signPos.y * scale : signPos.y, width: widthPx, transform: `rotate(${rotation}deg)` }} onMouseDown={pdfMode ? undefined : startSignDrag}>
      {!pdfMode && <button className="rotateHandle" title="Зажмите и поверните вывеску" onMouseDown={(e) => { e.stopPropagation(); setDrag({ type: 'rotate' }); }}>↻</button>}
      ...
      <div className="letters" style={{ fontFamily: font, color: face, fontSize, textShadow }}>
        {logo && <img className="logo" src={logo} style={{ height: fontSize, filter: night ? `drop-shadow(0 0 ${fontSize * 0.34}px ${face}) drop-shadow(${depthPx}px ${depthPx}px 0 ${side})` : `drop-shadow(${depthPx}px ${depthPx}px 0 ${side}) drop-shadow(${depthPx * 1.2}px ${depthPx * 1.2}px ${depthPx}px rgba(0,0,0,.26))` }}/>} 
  ...
  const CalibrationLine = ({ scale = 1, pdfMode = false }: { scale?: number; pdfMode?: boolean }) => {
    ...
    return points.length === 2 ? <svg className="measure"><defs><marker id={markerId} markerWidth="7" markerHeight="7" refX="3.5" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" fill="#ff6a00"/></marker></defs><line x1={points[0].x * scale} y1={points[0].y * scale} x2={points[1].x * scale} y2={points[1].y * scale} markerStart={`url(#${markerId})`} markerEnd={`url(#${markerId})`}/><text x={(points[0].x+points[1].x)/2 * scale} y={(points[0].y+points[1].y)/2 * scale-10}>{cm(calibrationCm)}</text></svg> : null;
  };
    ...
    <main className="app">
      ...
      <section className="workspace">
        ...
        <div className={'stage ' + (night ? 'night' : '')} ref={stageRef} onClick={clickStage}>
          ...
          {points.map((p, i) => <div key={i} className="point" style={{ left: p.x, top: p.y }} onMouseDown={(e) => startPointDrag(e, i)}>{i + 1}</div>)}
          <CalibrationLine />
          <SignGraphic />
        </div>
      </section>

      <aside className="panel right compact">
        ...
        <div className="summaryCard"><b>Габариты</b><span>Длина: {cm(signWidthCm)}</span><span>Высота: {cm(signHeightCm)}</span><span>Высота букв: {cm(letterHeightCm)}</span><span>Подсветка: {lightingLabel(lighting)}</span></div>
      </aside>
    </main>
    ...
    <div className="pdfPages" ref={pdfRef}>
      <section className="pdfSheet pdfSheetFacade">
        <header className="pdfTopHeader">
          ...
          <div className="pdfTrust"><i>▣</i><span>РАБОТАЕМ С<br/><b>2014 ГОДА</b></span></div>
      ...
      <section className="pdfSheet pdfSheetDrawing">
        ...
        <div className="pdfDrawingGrid">
          <div className="pdfDrawingStage">
            <SignGraphic pdfMode scale={Math.min(1.15, 900 / Math.max(1, signWidthPx))}/>
