...
function App() {
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    const substrateShadow = `${Math.max(2, depthPx)}px ${Math.max(2, depthPx)}px 0 ${substrateEdgeColor}, ${Math.max(8, depthPx * 6)}px ${Math.max(8, depthPx * 6)}px ${Math.max(12, depthPx * 10)}px rgba(0,0,0,.24)`;
    const substrateRadius = Math.max(12, Math.min(widthPx, heightPx) * 0.08);
    ...
    >
      ...
      >
        ...
        {drawingMode && pdfMode ? <>
          {drawHDim('sign-width', signAbsLeft, signAbsRight, dimHorizontalY, signAbsTop, mm(displayWidthMm), -10)}
          ...
          {drawHDim('live-sign-width', signAbsLeft, signAbsRight, dimHorizontalY, signAbsTop, mm(displayWidthMm), -8)}
