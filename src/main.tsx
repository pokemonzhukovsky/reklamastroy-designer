...
function App() {
  ...
  useEffect(() => {
    const move = (e: PointerEvent) => {
      ...
      // На телефоне не даём браузеру прокручивать страницу, пока пользователь
      // двигает вывеску, точки калибровки или ручку поворота.
  ...
  }, [drag, stageSize, signPos, visualSignWidthPx, visualSignHeightPx]);
  ...
  const startSignDrag = (e: React.PointerEvent<HTMLDivElement>) => {
  ...
  const startPointDrag = (e: React.PointerEvent<HTMLDivElement>, index: number) => {
    if (e.pointerType === 'mouse' && e.button !== 0) return;
    e.preventDefault();
    e.stopPropagation();
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}mm" height="${h}mm" viewBox="0 0 ${w} ${h}" version="1.1">
  ...
  const pdf = async () => {
    ...
    // КРИТИЧНО ДЛЯ PDF: геометрию текста нельзя считать до фактической загрузки
    // выбранного шрифта. После расширения списка Google Fonts браузер иногда успевал
    // измерить строку запасным шрифтом, а к моменту html2canvas уже рисовал настоящий.
    // В результате текст визуально «уезжал» вправо, а логотип оставался в координатах,
    // рассчитанных по старой ширине. На 2-й странице PDF это выглядело как разъехавшийся
    // макет. Поэтому сначала явно загружаем ТОЛЬКО активные шрифты, затем принудительно
    // пересчитываем все canvas-метрики и ждём два кадра React/браузера.
    const fontsApi = (document as any).fonts;
    if (fontsApi) {
      ...
      await Promise.all(activeFonts.map((family) =>
        fontsApi.load(`900 160px ${family}`).catch(() => undefined)
      ));
      flushSync(() => setFontMeasureTick(v => v + 1));
      await new Promise(requestAnimationFrame);
      await new Promise(requestAnimationFrame);
    }
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false, snapshotSrc = '' }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean; snapshotSrc?: string }) => {
    ...
    return <div
      ...
      onPointerDown={pdfMode ? undefined : startSignDrag}
    >
      {!pdfMode && <button className="rotateHandle" style={{ top: heightPx + 78, left: widthPx + 86, transform: 'none' }} title="Зажмите и поверните вывеску" onPointerDown={(e) => {
        e.preventDefault();
        e.stopPropagation();
        if (!stageRef.current) return;
        const p = getStagePoint(e.clientX, e.clientY);
    ...
    <main className="app constructorFrame">
      ...
      <section className="workspace">
        <div className={'stage ' + (night ? 'night' : '') + (facadeDragOver ? ' facadeDragOver' : '')} ref={stageRef} onClick={clickStage} onDragOver={onStageDragOver} onDragLeave={onStageDragLeave} onDrop={onStageDrop}>
          ...
          {points.map((p, i) => <div key={i} className="point" style={{ left: p.x, top: p.y }} onPointerDown={(e) => startPointDrag(e, i)} />)}
          <CalibrationLine />
          {facade && <SignGraphic />}
        </div>
      </section>
