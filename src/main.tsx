...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}mm" height="${h}mm" viewBox="0 0 ${w} ${h}" version="1.1">
  ...
  const pdf = async () => {
    ...
    // v3.72 — первая страница PDF содержит две высокодетализированные визуализации без JPEG-сжатия:
    // слева дневной режим, справа ночной. Обе снимаются из одной и той же живой геометрии,
    // поэтому положение, масштаб и поворот вывески остаются идентичными.
    const restoreNightAfterExport = night;
    const stageEl = stageRef.current;
    const exportScale = Math.max(4, Math.min(5, (window.devicePixelRatio || 1) * 2.5));
    ...
    const captureStageMode = async (nightMode: boolean) => {
      ...
      return {
        shot: exportCanvas.toDataURL('image/png'),
        ratio: exportCanvas.width && exportCanvas.height ? exportCanvas.width / exportCanvas.height : 16 / 9
      };
    };

    const makePdfModeLabel = (title: string, subtitle: string, accent: string, icon: string) => {
      ...
      const x = 4;
      const y = 6;
      const w = labelCanvas.width - 8;
      const h = labelCanvas.height - 12;
      ...
      ctx.fillText(icon, 48, 56);

      ctx.textAlign = 'left';
      ctx.fillStyle = '#1f2937';
      ctx.font = 'bold 34px Arial, Helvetica, sans-serif';
      ctx.fillText(title, 84, 44);
      ctx.fillStyle = '#6b7280';
      ctx.font = '600 22px Arial, Helvetica, sans-serif';
      ctx.fillText(subtitle, 84, 74);
      return labelCanvas.toDataURL('image/png');
    };

    // Сначала фиксируем день, затем ночь. Геометрию текста не пересчитываем между снимками.
    const dayCapture = await captureStageMode(false);
    const nightCapture = await captureStageMode(true);
    const shotDay = dayCapture.shot;
    const shotNight = nightCapture.shot;
    const ratio = dayCapture.ratio;
    const dayLabelShot = makePdfModeLabel('День', 'Дневная визуализация', '#f59e0b', '☀');
    const nightLabelShot = makePdfModeLabel('Ночь', 'Ночная визуализация', '#334155', '☾');

    // Возвращаем интерфейс пользователя в исходный режим сразу после получения двух снимков.
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (i === 0 && pdfStageMm && shotDay && shotNight) {
        const gap = 4;
        const halfW = (pdfStageMm.w - gap) / 2;
        const labelH = 18;
        const imageBoxH = pdfStageMm.h - labelH;
        const shotRatio = ratio || 16 / 9;

        const fitShot = (boxX: number) => {
          const boxY = pdfStageMm.y + labelH;
          const boxRatio = halfW / imageBoxH;
          let drawW = halfW;
          let drawH = imageBoxH;
          let drawX = boxX;
          let drawY = boxY;
          if (shotRatio >= boxRatio) {
            drawW = halfW;
            drawH = halfW / shotRatio;
            drawY = boxY + (imageBoxH - drawH) / 2;
          } else {
            drawH = imageBoxH;
            drawW = imageBoxH * shotRatio;
            drawX = boxX + (halfW - drawW) / 2;
          }
        ...
        const labelW = Math.max(54, halfW - 2);
        const labelImgH = 10.5;
        if (dayLabelShot) doc.addImage(dayLabelShot, 'PNG', pdfStageMm.x, pdfStageMm.y + 1.5, labelW, labelImgH, undefined, 'FAST');
        if (nightLabelShot) doc.addImage(nightLabelShot, 'PNG', pdfStageMm.x + halfW + gap, pdfStageMm.y + 1.5, labelW, labelImgH, undefined, 'FAST');

        const dayRect = fitShot(pdfStageMm.x);
        const nightRect = fitShot(pdfStageMm.x + halfW + gap);
        doc.addImage(shotDay, 'PNG', dayRect.drawX, dayRect.drawY, dayRect.drawW, dayRect.drawH, undefined, 'FAST');
        doc.addImage(shotNight, 'PNG', nightRect.drawX, nightRect.drawY, nightRect.drawW, nightRect.drawH, undefined, 'FAST');

        // Для водяного знака дальше используем общую область обоих снимков.
        const drawX = pdfStageMm.x;
        const drawY = pdfStageMm.y + labelH;
        const drawW = pdfStageMm.w;
        const drawH = imageBoxH;
  ...
  const drawingScale = Math.min(1.08, 900 / Math.max(1, visualSignWidthPx + 430), 390 / Math.max(1, visualSignHeightPx + 520));
    ...
    <div className="pdfPages" ref={pdfRef}>
      <section className="pdfSheet pdfSheetFacade">
        ...
        <div className="pdfFacadeCenter">
          <div className="pdfFacadeStage snapshotStage pdfFacadeCompare">
            {stageShotDay && stageShotNight ? <>
              <div className="pdfFacadeCompareItem">
                <div className="pdfFacadeModeLabel">День</div>
                <div className="stageSnapshotBox stageSnapshotBoxFixed" style={stageShotBoxStyle}>
                  <img src={stageShotDay} className="stageSnapshot"/>
                </div>
              </div>
              <div className="pdfFacadeCompareItem">
                <div className="pdfFacadeModeLabel">Ночь</div>
                <div className="stageSnapshotBox stageSnapshotBoxFixed" style={stageShotBoxStyle}>
                  <img src={stageShotNight} className="stageSnapshot"/>
                </div>
              </div>
