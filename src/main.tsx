...
function App() {
  ...
  const drawingScale = Math.min(1.16, 780 / Math.max(1, visualSignWidthPx + 260), 380 / Math.max(1, visualSignHeightPx + 170));
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    // Борт подложки не выбирается отдельно: он соответствует основному выбранному цвету подложки.
    const substrateEdgeColor = substrateColor;
    const substrateDepth = Math.max(5, Math.min(28, Math.min(widthPx, heightPx) * 0.095));
    const substrateShadow = `${substrateDepth}px ${substrateDepth}px 0 ${substrateEdgeColor}, ${substrateDepth * 1.45}px ${substrateDepth * 1.45}px ${substrateDepth * 1.8}px rgba(0,0,0,.38), ${substrateDepth * 0.35}px ${substrateDepth * 0.35}px ${substrateDepth * 0.75}px rgba(0,0,0,.26), inset ${Math.max(1, substrateDepth * 0.12)}px ${Math.max(1, substrateDepth * 0.12)}px 0 rgba(255,255,255,.22), inset -${Math.max(1, substrateDepth * 0.18)}px -${Math.max(1, substrateDepth * 0.18)}px 0 rgba(0,0,0,.13)`;
    ...
    const rightDimBase = Math.max(substrateAbsRight, signAbsRight) + (logo && logoPosition !== 'left' ? 120 : 60);
    ...
    >
      ...
      >
        ...
        {drawingMode && pdfMode ? <>
          ...
          {drawHDim('sign-width', signAbsLeft, signAbsRight, substrateEnabled ? signAbsBottom + 36 : dimHorizontalY, signAbsTop, mm(contentWidthMm), substrateEnabled ? 31 : -10)}
