...
function App() {
  ...
  const [pdfForceDay, setPdfForceDay] = useState(false);
  ...
  const renderNight = night && !pdfForceDay;
  ...
  const lightingTextStyle = (fs: number, currentFace = face, currentSide = side, flatAppearance = false) => {
  ...
  const logoLightingFilter = (basePx: number, depthPx: number, flatAppearance = false) => {
    if (flatAppearance) return 'none';
    if (!renderNight) return `drop-shadow(${depthPx}px ${depthPx}px 0 ${side}) drop-shadow(${depthPx * 1.2}px ${depthPx * 1.2}px ${depthPx}px rgba(0,0,0,.26))`;
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    const substrateNightOnly = renderNight && !pdfMode;
    ...
    const substrateDepth = Math.max(10, Math.min(40, Math.min(widthPx, heightPx) * 0.1));
    const substrateDepthOffset = Math.max(5, Math.min(20, substrateDepth * 0.52));
    const substrateDepthColor = flatPdfAppearance ? substrateColor : shadeHex(substrateColor, substrateNightOnly ? -120 : -64);
    const substrateCastShadow = flatPdfAppearance
      ? 'none'
      : (substrateNightOnly
        ? `${substrateDepth * 0.56}px ${substrateDepth * 0.82}px ${substrateDepth * 1.9}px rgba(0,0,0,.68), ${substrateDepth * 0.9}px ${substrateDepth * 1.2}px ${substrateDepth * 2.6}px rgba(0,0,0,.48)`
        : `${substrateDepth * 0.45}px ${substrateDepth * 0.62}px 0 rgba(0,0,0,.18), ${substrateDepth * 0.72}px ${substrateDepth * 0.96}px ${substrateDepth * 1.2}px rgba(0,0,0,.22), ${substrateDepth * 1.05}px ${substrateDepth * 1.36}px ${substrateDepth * 2.0}px rgba(0,0,0,.30)`);
  ...
  return <>
    <main className="app constructorFrame">
      ...
      <section className="workspace">
        <div className={'stage ' + (renderNight ? 'night' : '') + (facadeDragOver ? ' facadeDragOver' : '')} ref={stageRef} onClick={clickStage} onDragOver={onStageDragOver} onDragLeave={onStageDragLeave} onDrop={onStageDrop}>
