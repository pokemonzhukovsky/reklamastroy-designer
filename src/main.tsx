...
function App() {
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    const dimSignX = corelDims ? 105 : 70;
    const dimTopY = corelDims ? 60 : 48;
    const dimHorizontalY = corelDims ? 34 : 26;
    const capArrowX = corelDims ? 64 : 48;
    const lowerArrowX = corelDims ? 34 : 22;
    ...
    const dimSvgWidth = widthPx + (corelDims ? 210 : 140);
    const dimSvgHeight = heightPx + (corelDims ? 118 : 88);
    return <div
    ...
    >
      ...
      <svg className="signDims" width={dimSvgWidth} height={dimSvgHeight} viewBox={`0 0 ${dimSvgWidth} ${dimSvgHeight}`}>
        <defs>
