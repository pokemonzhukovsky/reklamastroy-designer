...
function App() {
  ...
  const leadSubject = 'Заявка на расчет вывески из конструктора';
  const leadBody = [
    'Здравствуйте! Прошу рассчитать вывеску по макету из конструктора.',
    '',
    `Текст 1 строки: ${line1 || 'не указан'}`,
    twoLines ? `Текст 2 строки: ${line2 || 'не указан'}` : '',
    `Длина вывески: ${mm(signWidthCm)}`,
    `Высота вывески: ${mm(displayHeightMm)}`,
    `Высота 1 строки: ${mm(line1HeightCm)}`,
    twoLines ? `Высота 2 строки: ${mm(line2HeightCm)}` : '',
    logo ? `Высота логотипа: ${mm(logoHeightCm)}` : '',
    `Подсветка: ${lightingLabel(lighting)}`,
    `Цвет лица: ${colorName(ORACAL_8500, face)}`,
    `Цвет бортов: ${colorName(ORACAL_641, side)}`,
    '',
    'Прикреплю PDF-макет из конструктора.'
  ].filter(Boolean).join('\n');
  const leadMailLink = `mailto:sales@reklamastroy.ru?subject=${encodeURIComponent(leadSubject)}&body=${encodeURIComponent(leadBody)}`;
  ...
  return <>
    <header className="topbar topbarSales">
      <div className="brand"><b>Реклама<span>Строй</span></b><small>онлайн примерка вывески</small></div>
      <div className="heroTitle"><h1>Соберите макет вывески и получите расчет</h1><p>Фасад, масштаб, цвета ORACAL и PDF для менеджера — в одном конструкторе.</p></div>
      <div className="topbarLead">
        <span>Нужна помощь?</span>
        <a href="tel:+74950083795">+7 495 008 37 95</a>
      </div>
    </header>

    <main className="app">
      <aside className="panel left panelSales">
        <div className="conversionHero">
          <b>Получите расчет быстрее</b>
          <span>Соберите макет, скачайте PDF и отправьте его менеджеру — так расчет будет точнее.</span>
        </div>
        <div className="salesSteps">
          <span>Фото</span><span>Размер</span><span>Макет</span><span>Заявка</span>
        </div>
        ...
        <div className="summaryCard leftSummary"><b>Итоговые параметры</b><span>Длина: {mm(signWidthCm)}</span><span>Высота: {mm(displayHeightMm)}</span><span>1 строка: {mm(line1HeightCm)}</span>{twoLines && <span>2 строка: {mm(line2HeightCm)}</span>}{logo && <span>Высота логотипа: {mm(logoHeightCm)}</span>}<span>Подсветка: {lightingLabel(lighting)}</span></div>
        <div className="leadCard">
          <b>Готово к расчету</b>
          <span>Скачайте PDF и отправьте заявку. Менеджер увидит фасад, размеры, цвета и макет.</span>
          <button className="panelPdfButton" onClick={pdf}>Скачать PDF для заявки</button>
          <a className="leadPrimary" href={leadMailLink}>Отправить заявку на расчет</a>
          <a className="leadSecondary" href="tel:+79258883795">Позвонить менеджеру</a>
        </div>
      </aside>

      <section className="workspace">
        <div className="toolbar toolbarSales">
          ...
          <div className="workspaceHint">Перетащите вывеску мышкой. Для поворота используйте круглую ручку.</div>
          <a className="toolbarLead" href={leadMailLink}>Отправить на расчет</a>
        </div>
