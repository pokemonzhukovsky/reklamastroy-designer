...
function App() {
  ...
  // Ширина технического макета должна совпадать с фактической DOM-отрисовкой.
  // У .letters задан letter-spacing: .01em, поэтому учитываем этот интервал и в геометрии.
  const textLineWidthsPx = lineTextMetrics.map((metric, i) => {
    const chars = Array.from(lines[i] || '').length;
    const letterSpacingPx = Math.max(0, chars - 1) * lineFontSizesPx[i] * 0.01;
    return (metric.visualWidth || metric.width) + letterSpacingPx;
  });
    ...
    >
      ...
      <div className="letters" style={lettersStyle} data-lighting={lighting}>
        <>
            ...
            <div className="textBlock textBlockAbsolute" style={{ left: textStartX, top: textTopY, width: textWidthPxScaled, height: textBlockHeightPx, gap: 0, overflow: 'visible' }}>{lines.map((line, i) => <span key={i} style={{ marginLeft: `${lineOffsetsScaled[i] || 0}px`, marginTop: i > 0 ? `${lineGapPxScaled}px` : 0, fontSize: fontSizes[i], fontFamily: lineFonts[i] || font, lineHeight: `${lineTargetHeightsPx[i] * scale}px`, minHeight: `${lineTargetHeightsPx[i] * scale}px`, width: `${lineWidthsScaled[i] || 0}px`, overflow: 'visible', whiteSpace: 'nowrap', transform: 'none', ...lightingTextStyle(fontSizes[i], lineFaces[i] || face, lineSides[i] || side) }}>{line}</span>)}</div>
        </>
      </div>
    </div>;
  };
    ...
    {downloadGateOpen && createPortal(
      <div className="leadGateOverlay" role="dialog" aria-modal="true" aria-labelledby="lead-gate-title" onMouseDown={(e) => {
        ...
        <div className="leadGateModal">
          ...
          <p>Введите имя и телефон. После отправки заявки файл {pendingDownload === 'svg' ? 'SVG' : 'PDF'} начнёт скачиваться автоматически.</p>
