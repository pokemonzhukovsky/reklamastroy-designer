...
function App() {
  ...
  const pdf = async () => {
    ...
    // Запоминаем реальное положение фотографии фасада внутри рабочей области ДО экспорта.
    // В рабочем окне фасад выводится через object-fit: contain, поэтому внутри stage могут
    // быть серые поля. Если потом вставлять в PDF не саму область фотографии, а весь stage
    // или пересобирать фасад заново, вывеска визуально съезжает вправо/влево относительно фото.
    // Поэтому экспортируем именно видимый прямоугольник фотографии вместе с наложенной вывеской.
    ...
    const liveFacadeRect = liveFacadeImg?.getBoundingClientRect() || null;
    ...
    // Вырезаем из снимка stage только реальную фотографию фасада.
    // Это сохраняет положение вывески точно таким, каким пользователь видит его в рабочем окне,
    // потому что вывеска уже находится внутри этого же растрового снимка.
    ...
    if (liveFacadeRect && liveStageRect.width > 0 && liveStageRect.height > 0 && liveFacadeRect.width > 10 && liveFacadeRect.height > 10) {
      ...
      const sx = Math.max(0, Math.round((liveFacadeRect.left - liveStageRect.left) * scaleX));
      const sy = Math.max(0, Math.round((liveFacadeRect.top - liveStageRect.top) * scaleY));
      const sw = Math.min(stageCanvas.width - sx, Math.round(liveFacadeRect.width * scaleX));
      const sh = Math.min(stageCanvas.height - sy, Math.round(liveFacadeRect.height * scaleY));
      if (sw > 20 && sh > 20) {
