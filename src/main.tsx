...
function ColorDropdown({
  ...
  const selected = list.find(c => c[2].toLowerCase() === value.toLowerCase()) || list[0];

  return (
    <div className="oracalDropdown">
      <label>{label}</label>
      <button className="oracalCurrent" type="button" onClick={() => setOpen(v => !v)}>
        <span className="oracalSwatch" style={{ background: selected[2] }} />
        <span className="oracalInfo">
          <b>{selected[0]} — {selected[1]}</b>
          <small>Нажмите, чтобы выбрать цвет</small>
        </span>
      </button>
      {open && (
        <div className="oracalMenu">
          {list.map(c => (
            <button
              type="button"
              key={`${label}-${c[0]}-${c[1]}`}
              className={value.toLowerCase() === c[2].toLowerCase() ? 'selected' : ''}
              onClick={() => { onChange(c[2]); setOpen(false); }}
            >
              <span className="oracalSwatch" style={{ background: c[2] }} />
              <span className="oracalInfo">
                <b>{c[0]} — {c[1]}</b>
                <small>{c[2].toUpperCase()}</small>
              </span>
            </button>
          ))}
        </div>
...
function App() {
  ...
  const dimHex = (hex: string, k = 0.36) => {
  ...
  const lightingTextStyle = (fs: number) => {
    ...
    const faceGlow = `0 0 ${fs * 0.16}px ${face}, 0 0 ${fs * 0.28}px ${face}`;
    const sideGlow = `0 0 ${fs * 0.18}px ${side}, 0 0 ${fs * 0.34}px ${side}`;
    const backHalo = `0 0 ${fs * 0.42}px ${whiteGlow}, 0 0 ${fs * 0.72}px ${whiteGlow}`;

    // Ночь: каждый тип подсветки отображает только свой источник света.
    if (lighting === 'front') {
      // Лицевая: светится только лицевая часть, без ореола вокруг вывески.
      return { color: face, textShadow: 'none', WebkitTextStroke: `${strokeBase}px ${darkSide}` } as React.CSSProperties;
    }
    if (lighting === 'side') {
      // Торцевая: лицевая часть несветовая в ORACAL 8500, светятся только борта ORACAL 641.
      return { color: face, textShadow: sideGlow, WebkitTextStroke: `${Math.max(1.2, strokeBase * 1.75)}px ${side}` } as React.CSSProperties;
    }
    if (lighting === 'back') {
      // Контражур: лицевая часть несветовая в ORACAL 8500, задний ореол белый.
      return { color: face, textShadow: backHalo, WebkitTextStroke: `${strokeBase}px ${darkSide}` } as React.CSSProperties;
    }
    if (lighting === 'frontBack') {
      // Лицевая + контражур: лицевая ORACAL 8500 + белое заднее свечение.
      return { color: face, textShadow: `${faceGlow}, ${backHalo}`, WebkitTextStroke: `${strokeBase}px ${darkSide}` } as React.CSSProperties;
    }
    if (lighting === 'frontSide') {
      // Лицевая + торцевая: лицо ORACAL 8500, торец ORACAL 641.
      return { color: face, textShadow: `${faceGlow}, ${sideGlow}`, WebkitTextStroke: `${Math.max(1, strokeBase * 1.45)}px ${side}` } as React.CSSProperties;
    }
    // Без подсветки: вся вывеска равномерно затемняется и не светится.
    return { color: dimHex(face), textShadow: 'none', WebkitTextStroke: `${strokeBase}px ${dimHex(side)}` } as React.CSSProperties;
  };
  const logoLightingFilter = (basePx: number, depthPx: number) => {
    ...
    if (lighting === 'front') return `drop-shadow(${depthPx}px ${depthPx}px 0 ${darkSide})`;
    if (lighting === 'side') return `drop-shadow(0 0 ${basePx * 0.24}px ${side}) drop-shadow(${depthPx}px ${depthPx}px 0 ${side})`;
    if (lighting === 'back') return `drop-shadow(0 0 ${basePx * 0.58}px ${whiteGlow}) drop-shadow(0 0 ${basePx * 0.95}px ${whiteGlow})`;
    if (lighting === 'frontBack') return `drop-shadow(0 0 ${basePx * 0.24}px ${face}) drop-shadow(0 0 ${basePx * 0.62}px ${whiteGlow}) drop-shadow(${depthPx}px ${depthPx}px 0 ${darkSide})`;
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    >
      ...
      <svg
        ...
        width={widthPx + 132}
        height={heightPx + 88}
        viewBox={`0 0 ${widthPx + 132} ${heightPx + 88}`}
        style={{ left: -66, top: -42 }}
      >
        <defs>
        ...
        {drawingMode && <>
          <line className="dimExt" x1="66" y1="26" x2="66" y2="48" />
          <line className="dimExt" x1={widthPx + 66} y1="26" x2={widthPx + 66} y2="48" />
          <line className="dimExt" x1="42" y1="48" x2="66" y2="48" />
          <line className="dimExt" x1="42" y1={heightPx + 48} x2="66" y2={heightPx + 48} />
        </>}
        <line x1="66" y1="26" x2={widthPx + 66} y2="26" markerStart={`url(#arrow-start-${pdfMode ? 'pdf' : 'live'})`} markerEnd={`url(#arrow-end-${pdfMode ? 'pdf' : 'live'})`} />
        <text x={widthPx / 2 + 66} y="18" textAnchor="middle">{cm(signWidthCm)}</text>
        <line x1="42" y1="48" x2="42" y2={heightPx + 48} markerStart={`url(#arrow-start-${pdfMode ? 'pdf' : 'live'})`} markerEnd={`url(#arrow-end-${pdfMode ? 'pdf' : 'live'})`} />
        <text x="20" y={heightPx / 2 + 54} textAnchor="middle" transform={`rotate(-90 20 ${heightPx / 2 + 54})`}>{cm(signHeightCm)}</text>
      </svg>
  ...
  return <>
    ...
    <main className="app">
      <aside className="panel left">
        ...
        <ColorDropdown label="Лицевая часть · ORACAL 8500" list={ORACAL_8500} value={face} onChange={setFace} />
        <ColorDropdown label="Борта · ORACAL 641" list={ORACAL_641} value={side} onChange={setSide} />
