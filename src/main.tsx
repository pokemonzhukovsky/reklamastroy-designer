...
function App() {
  ...
  // Для рабочего поля важна не оптическая компенсация шрифта, а строгое совпадение
  // точки начала текста. Поэтому при выравнивании «слева» обе строки получают
  // одинаковый X без поправок на side-bearing конкретной буквы.
  const textLineWidthsPx = lineTextMetrics.map(metric => metric.width);
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    const dimSvgHeight = heightPx + (corelDims ? 520 : 120);
    ...
    const signWidthDimY = substrateEnabled
      ? (twoLines && lineAbs[1] ? Math.max(signAbsBottom + 112, line2WidthDimY + 96) : signAbsBottom + 78)
      : dimHorizontalY;
    return <div
    ...
    >
      ...
      >
        ...
        {drawingMode && pdfMode ? <>
          ...
          {drawHDim('sign-width', signAbsLeft, signAbsRight, signWidthDimY, signWidthDimY > signAbsBottom ? signAbsBottom : signAbsTop, mm(contentWidthMm), substrateEnabled ? 31 : -10)}
