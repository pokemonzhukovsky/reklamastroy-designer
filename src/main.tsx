...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}mm" height="${h}mm" viewBox="0 0 ${w} ${h}" version="1.1">
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false, snapshotSrc = '' }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean; snapshotSrc?: string }) => {
    ...
    // Рабочая область: отдельно показываем высоту строк именно тем числом,
    // которое пользователь ввёл в поле "Высота строки, мм".
    // Общий габарит вывески может отличаться из-за второй строки, логотипа,
    // зазора или подложки, но размер буквы должен оставаться прямой
    // производственной величиной и не пересчитываться по фактическому bbox.
    const liveMainHeightLabel = !substrateEnabled && !logo && !twoLines ? line1HeightCm : contentHeightMm;
    ...
    >
      ...
      >
        ...
        {drawingMode && pdfMode ? <>
          ...
          {drawVDim('live-sign-height', substrateEnabled ? substrateAbsRight + 22 : signAbsLeft - 22, signAbsTop, signAbsBottom, substrateEnabled ? signAbsRight : signAbsLeft, mm(liveMainHeightLabel), substrateEnabled ? 'right' : 'left')}
          {lineAbs[0] && drawVDim('live-line1-height', line1CapX, lineAbs[0].top, lineAbs[0].bottom, lineAbs[0].right, mm(line1HeightCm), 'right', { horizontalLabel: true, labelY: lineAbs[0].top + 18, textOffset: 9 })}
