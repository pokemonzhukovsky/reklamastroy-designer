...
function App() {
  ...
  }, [drag, stageSize, signPos, signWidthPx, signHeightPx]);
  ...
  const startSignDrag = (e: React.MouseEvent<HTMLDivElement>) => {
    if (e.button !== 0 || placingPoints || !stageRef.current) return;
    e.stopPropagation();
    const p = getStagePoint(e.clientX, e.clientY);
  ...
  const SignGraphic = ({ pdfMode = false, scale = 1, drawingMode = false }: { pdfMode?: boolean; scale?: number; drawingMode?: boolean }) => {
    ...
    >
      {!pdfMode && <button className="rotateHandle" title="Зажмите и поверните вывеску" onMouseDown={(e) => {
        e.stopPropagation();
        if (!stageRef.current) return;
        const p = getStagePoint(e.clientX, e.clientY);
        const cx = signPos.x + signWidthPx / 2;
      ...
      <div className="letters" style={{ fontFamily: font, color: face, fontSize, textShadow }}>
        {logo && <img className="logo" src={logo} draggable={false} style={{ height: fontSize, filter: night ? `drop-shadow(0 0 ${fontSize * 0.34}px ${face}) drop-shadow(${depthPx}px ${depthPx}px 0 ${side})` : `drop-shadow(${depthPx}px ${depthPx}px 0 ${side}) drop-shadow(${depthPx * 1.2}px ${depthPx * 1.2}px ${depthPx}px rgba(0,0,0,.26))` }}/>} 
  ...
  return <>
    ...
    <main className="app">
      <aside className="panel left">
        ...
        <div className="summaryCard leftSummary"><b>Габариты</b><span>Длина: {cm(signWidthCm)}</span><span>Высота: {cm(signHeightCm)}</span><span>Высота букв: {cm(letterHeightCm)}</span><span>Подсветка: {lightingLabel(lighting)}</span></div>
      </aside>
